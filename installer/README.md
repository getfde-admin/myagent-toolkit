...
这一包是「安装一只小Agent」时要种进**使用者 instance repo** 的档案，等价于官方安装器种入 `yh_agent` 的内容。
由 `myAgentAdmin` 前端 bundle 后，透过 GitHub Git Data API 一次 commit 进使用者 repo。

反推与设计说明见：[`_DOCS/myAgentAdmin/05_worker-package-and-workflows-reverse-engineered.md`](../../_DOCS/myAgentAdmin/05_worker-package-and-workflows-reverse-engineered.md)
...
│   ├── deploy-myagent.yml          # 部署（Terraform + CF Worker + D1 + Telegram webhook）
...
│   ├── issue-1.yml                        # 执行小Agent任务 #1
│   └── lib/{worker-name.sh, inject-workflow-env.sh}
└── templates/
    ├── .githubagent-init                   # 初始化标记档
...
- `deploy-myagent.yml` 会抓 `worker-package.zip`（来自 myAgentCore GitHub Pages）并依赖 `Terraform/` output 名称 —— 部署前必须先建好 `myAgentCore` 并发布（见 05 待办）。
- `actions/error-handler-action@v1`、`update-comment-action@v1`（`issue-1.yml` 用）需 `myAgentToolkit` repo 已打 `v1` tag 才引用得到。
